...
XMLReader::XMLReader(const std::string &fileName)
{
	...
	ROS_INFO("Attempting to load XML file: %s", fileName.c_str());
    tinyxml2::XMLError error = doc->LoadFile(fileName.c_str());
    if (error != tinyxml2::XML_SUCCESS) {
        ROS_ERROR("File problem! Error code: %d", error);
        throw std::runtime_error("Error opening input XML file");
    }
    ROS_INFO("XML file loaded successfully");
    root = doc->FirstChildElement("root");
    if (!root) {
        ROS_ERROR("No root element in XML");
        throw std::runtime_error("Invalid XML structure");
    }
    ROS_INFO("Root element found");
}
